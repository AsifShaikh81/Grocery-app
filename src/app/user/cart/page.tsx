...
function CartPage() {
  ...
  return (
    <div className='w-[95%] sm:w-[90%] md:w-[80%] mx-auto mt-8 mb-24 relative'>
      <Link href={"/"} className='absolute -top-2 left-0 flex items-center gap-2 text-[#F25A1A] hover:text-[#D94E14] font-medium transition-all'>
         <ArrowLeft size={20}/>
...
className='text-2xl sm:text-3xl md:text-4xl font-bold text-[#F25A1A] text-center mb-10'
...
className='text-center py-20 bg-white rounded-2xl shadow-md'
 >
  ...
  <Link href={"/"} className='bg-[#F25A1A] text-white px-6 py-3 rounded-full hover:bg-[#D94E14] transition-all inline-block font-medium'>Continue Shopping</Link> 
                ...
                >
                    <div className='relative w-28 h-28 sm:w-24 sm:h-24 md:w-28 md:h-28 flex-shrink-0 rounded-xl overflow-hidden bg-gray-50'>
                     ...
                     <div className='mt-4 sm:mt-0 sm:ml-4 flex-1 text-center sm:text-left'>
                        ...
                        <p className='text-[#F25A1A] font-bold mt-1 text-sm sm:text-base'>₹{Number(item.price)*item.quantity}</p>
                     </div>
                     <div className='flex items-center justify-center sm:justify-end gap-3 mt-3 sm:mt-0 bg-gray-50 px-3 py-2 rounded-full'>
                       <button className='bg-white p-1.5 rounded-full hover:bg-[#D94E14] transition-all border border-gray-200' onClick={()=>dispatch(decreaseQuantity(item._id))}><Minus size={14} className='text-[#F25A1A]'/></button>
                       <span className='font-semibold text-gray-800 w-6 text-center'>{item.quantity}</span>
                       <button className='bg-white p-1.5 rounded-full hover:bg-[#D94E14] transition-all border border-gray-200' onClick={()=>dispatch(increaseQuantity(item._id))}><Plus size={14} className='text-[#F25A1A]'/></button>
                     </div>
...
<div className='space-y-3 text-gray-700 text-sm sm:text-base'>
    <div className='flex justify-between'>
        <span>Subtotal</span>
        <span className='text-[#F25A1A] font-semibold'>₹{subTotal}</span>
    </div>
    <div className='flex justify-between'>
        <span>Delivery Fee</span>
        <span className='text-[#F25A1A] font-semibold'>₹{deliveryFee}</span>
    </div>
    <hr className='my-3'/>
<div className='flex justify-between font-bold text-lg sm:text-xl'>
        <span>Final Total</span>
        <span className='text-[#F25A1A] font-semibold'>₹{finalTotal}</span>
    </div>
</div>
<motion.button whileTap={{scale:0.95}} className='w-full mt-6 bg-[#F25A1A] text-white py-3 rounded-full hover:bg-[#D94E14]transition-all font-semibold text-sm sm:text-base' onClick={()=>router.push("/user/checkout")}>
    Proceed to Checkout
</motion.button>
      </motion.div>
    </div>
