...
useEffect(()=>{
  ...
  return (
    <div className='w-full min-h-screen bg-linear-to-b from-[#F25A1A] to-white'>
      <div className='max-w-2xl mx-auto pb-24'>
            <div className='sticky top-0 bg-white/80 backdrop-blur-xl p-4 border-b shadow flex gap-3 items-center z-999'>
              <button className='p-2 bg-[#f15d1eaf] rounded-full' onClick={()=>router.back()}><ArrowLeft className="text-[#D94E14]" size={20} /></button>
              <div>
<h2 className='text-xl font-bold'>Track Order</h2>
<p className='text-sm text-gray-600'>order#{order?._id?.toString().slice(-6)} <span className='text-[#F25A1A] font-semibold'>{order?.status}</span></p>
              </div>
...
onClick={getSuggestion}
      ...
      <div className='flex gap-2 flex-wrap mb-3'>
        {suggestions.map((s, i) => (
          <motion.div
            key={s}
            whileTap={{ scale: 0.92 }}
            className="px-3 py-1 text-xs bg-[#F25A1A] border border-[#F25A1A] cursor-pointer text-white rounded-full"
              ...
              <div  className={`px-4 py-2 max-w-[75%] rounded-2xl shadow 
                  ${
                    msg.senderId.toString() === userData?._id
                      ? "bg-[#F25A1A] text-white rounded-br-none"
      ...
      <div className='flex gap-2 mt-3 border-t pt-3'>
        <input type="text" placeholder='Type a Message...' className='flex-1 bg-gray-100 px-4 py-2 rounded-xl outline-none focus:ring-2 focus:ring-[#F25A1A]' value={newMessage} onChange={(e) => setNewMessage(e.target.value)} />
        <button className='bg-[#F25A1A] hover:bg-[#D94E14] p-3 rounded-xl text-white' onClick={sendMsg}><Send size={18} /></button>
      </div>
