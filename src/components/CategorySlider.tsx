...
function CategorySlider() {
  ...
  return (
    ...
    >
      <h2 className="text-2xl md:text-3xl font-bold text-[#F25A1A] mb-6 text-center">
        🛒 Shop by Category
      </h2>
      {showLeft && (
        <button
          className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white shadow-lg hover:bg-[#F7F2D7] rounded-full w-10 h-10 flex items-center justify-center transition-all"
          onClick={() => scroll("left")}
        >
          <ChevronLeft className="w-6 h-6 text-[#F25A1A]" />
        </button>
      )}
          ...
          return (
            ...
            >
              <div className="flex flex-col items-center justify-center p-5">
                <Icon className="w-10 h-10 text-[#F25A1A] mb-3" />
      ...
      {showRight && (
        <button
          className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white shadow-lg hover:bg-[#F7F2D7] rounded-full w-10 h-10 flex items-center justify-center transition-all"
          onClick={() => scroll("right")}
        >
          <ChevronRight className="w-6 h-6 text-[#F25A1A]" />
        </button>
      )}
    </motion.div>
  );
