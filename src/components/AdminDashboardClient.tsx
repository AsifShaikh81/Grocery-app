    ...
    <div className='pt-28 w-[90%] md:w-[80%] mx-auto'>
      <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-10 text-center sm:text-left'>
        <motion.h1
          ...
          className="text-3xl md:text-4xl font-bold text-[#F25A1A]"

        >
          🏪 Admin Dashboard
        </motion.h1>

        <select className='border border-gray-300 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-[#F25A1A] outline-none transition w-full sm:w-auto'
      ...
      <motion.div
        ...
        className="bg-[#f15d1eaf] border border-[#F25A1A] shadow-sm rounded-2xl p-6 text-center mb-10"

      >
        <h2 className='text-lg font-semibold text-[#F25A1A] mb-2'>{title}</h2>
        <p className='text-4xl font-extrabold text-[#F25A1A]'>{currenEarning.toLocaleString()}</p>
      </motion.div>

      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10'>
        {stats.map((s, i) => {
          const icons = [
            <Package key="p" className="text-[#F25A1A] w-6 h-6" />,
            <Users key="u" className="text-[#F25A1A] w-6 h-6" />,
            <Truck key="t" className="text-[#F25A1A] w-6 h-6" />,
            <IndianRupee key="r" className="text-[#F25A1A] w-6 h-6" />,
          ...
          >
            <div className='bg-[#f15d1eaf] p-3 rounded-xl'>
             {icons[i]}
            </div>
            <div>
