...
function Nav({ user }: { user: IUser }) {
  ...
  const sideBar = menuOpen
    ? createPortal(
        <AnimatePresence>
          <motion.div
            ...
            className="fixed top-0 left-0 h-full w-[75%] sm:w-[60%] z-999 backdrop-blur-xl border-r shadow-[0_0_50px_-10px_rgba(0,255,100,0.3)] flex flex-col p-6 text-white bg-[#F25A1A]"
          >
            <div className="flex justify-between items-center mb-2">
              ...
              <button
                className="text-white/80 hover:text-[#F25A1A] text-2xl font-bold transition"
            ...
            <div className="flex items-center gap-3 p-3 mt-3 rounded-xl bg-white/10 hover:bg-white/15 transition-all shadow-inner">
              <div className="relative w-12 h-12 rounded-full overflow-hidden border-2 border-[#F25A1A] shadow-lg">
                {user.image ? (
                  <Image
                    src={user.image}
                    alt="user"
              ...
              <div>
                ...
                <p className="text-xs capitalize tracking-wide text-[#F7F2D7]">
                  {user.role}
                </p>
              </div>
            </div>
    ...
    : null;
  return (
    <div className="w-[95%] fixed top-4 left-1/2  -translate-x-1/2  rounded-2xl shadow-lg shadow-black/30 flex justify-between items-center h-20 px-4 md:px-8 z-50 md:px-8 z-50 bg-[#F25A1A]">
      {/* left section */}
      <Link
        href={"/"}
